...
Read the **`slide-authoring`** skill before writing — file contract, canvas, type scale, spacing, assets.

**Required:** paste **`PageLayout` + `DeckFooter`** from `slide-authoring` → `references/page-layout-template.md` into the deck (local TSX, not `@open-slide/core`). Wrap each page in `<PageLayout head={…} bodyAlign={…} footerLabel="…">` unless the page is a deliberate full-bleed exception.

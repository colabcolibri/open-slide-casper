...
**Every new `index.tsx` must include the head / body / footer layout** copied from `references/page-layout-template.md` (local components — not exported from `@open-slide/core`). Head and footer stay pinned; the body row is `1fr` with optional `bodyAlign`: `start` | `center` | `end`. Customize `DeckFooter`, `Eyebrow`, and title components inside the same file.
...
Copy **`references/page-layout-template.md`** into every new deck first (`PageLayout`, `DeckFooter`), then add typography helpers and pages. Minimal skeleton:

```tsx
import type { DesignSystem, Page, SlideMeta } from '@open-slide/core';
import { useSlidePageNumber } from '@open-slide/core';
// …PageLayout + DeckFooter from page-layout-template.md…
...
Full paste-ready `PageLayout` source: **`references/page-layout-template.md`**.

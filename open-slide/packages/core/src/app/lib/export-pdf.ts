...
@media print {
  ...
  /* Supersample: Chrome rasterizes filtered/composited layers (e.g. filter:
     ...
     gradient on a ${width}×${height} page bakes in at ~1× DPI and bands when the PDF
     is viewed scaled up. zoom:2 doubles the layer raster size; scale(0.5)
     composites it back to ${width}×${height}. Vector content (text, plain CSS
...
export async function exportSlideAsPdf(
  ...
  style.textContent = buildPrintStyles(canvas.width, canvas.height);
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    inner.style.width = frameSize.width;
    inner.style.height = frameSize.height;

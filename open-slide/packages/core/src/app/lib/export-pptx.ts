...
import { canvasSizeForModule, canvasSizeToPptxEmu, type SlideModule } from './sdk';
...
export async function exportSlideAsImagePptx(
  ...
  const emu = canvasSizeToPptxEmu(canvas);
  ...
  try {
    const pngBlobs = await captureSlidePagesAsPngBlobs(slide, (current, pageTotal) => {
      onProgress?.({
        ...
        current,
        total: pageTotal,
        percent: Math.min(95, (current / pageTotal) * 95),
      });
    });

    const images = await Promise.all(
      pngBlobs.map((blob) => blob.arrayBuffer().then((ab) => new Uint8Array(ab))),
    );

    onProgress?.({ phase: 'generating', current: total, total, percent: 98 });
    const pptx = await buildImagePptx(images, emu);
    downloadBlob(
...
async function buildImagePptx(
  ...
  files['ppt/presentation.xml'] = strToU8(presentationXml(n, emu));
  ...
  for (let i = 0; i < n; i++) {
    const idx = i + 1;
    files[`ppt/slides/slide${idx}.xml`] = strToU8(slideXml(emu));

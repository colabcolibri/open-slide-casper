...
import type { RasterExportProgress } from '../lib/export-raster';
import { Progress } from './ui/progress';

export function RasterExportProgressToast({ progress }: { progress: RasterExportProgress }) {
  const t = useLocale();
  const text =
    progress.phase === 'processing'
      ? format(t.rasterToast.processing, {
          current: progress.current.toString().padStart(2, '0'),
          total: progress.total.toString().padStart(2, '0'),
        })
      : progress.phase === 'generating'
        ? t.rasterToast.generating
        : t.rasterToast.done;

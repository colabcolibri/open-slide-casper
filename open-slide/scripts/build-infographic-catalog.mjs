#!/usr/bin/env node
/**
 * Build catalog.json, layouts-by-category.md, layouts/*.md + styles/*.md from vendored snapshot/.
...
function buildLayoutsByCategoryMarkdown(catalogLayouts) {
  const lines = [
    '# Layout archetypes by category',
    '',
    `**${catalogLayouts.length}** layout ids. Index: **\`catalog.json\`**. Prompt: **\`layouts/<id>.md\`**. Preview: **\`previews/layouts/<id>.webp\`**.`,
    '',
  ...
  for (const category of LAYOUT_TAXONOMY) {
    const items = catalogLayouts
      .filter((l) => (l.categories ?? []).includes(category))
      .sort((a, b) => a.id.localeCompare(b.id));
    lines.push(`## ${category} (${items.length})`, '', '| id | Name |', '| --- | --- |');
    for (const l of items) {
      lines.push(`| \`${l.id}\` | ${l.name} |`);
    }
    lines.push('');
  }

  lines.push('## Alphabetical ids', '', '');
  lines.push(
    catalogLayouts
      .map((l) => `\`${l.id}\``)
      .sort()
...
async function main() {
  ...
  await fs.writeFile(
    path.join(OUT, 'layouts-by-category.md'),
    buildLayoutsByCategoryMarkdown(catalogLayouts),
  );

  console.log(
    `built ${catalogLayouts.length} layouts, ${catalogStyles.length} styles (+ layouts-by-category.md)`,
  );
}

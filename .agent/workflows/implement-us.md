...
5. **Hard block:** US must be in a **planned** or **active** sprint (`implement-gate` / `sprint_stories`) — if not → stop; recommend `/plan-sprint`
6. Read every **Architecture refs** section before Write on code
...
2. Run implement gate checklist (architecture, ready, sprint membership, Plan, depends_on, status)
...
Typical flow: `/create-us` → `/plan-sprint` (add US to sprint) → `/refine-us` → **`/implement-us`** → `/complete-us`.
